...
graf = classes.Graf.llegir_fitxer(nom_graf)

vectors = []    # lista de los vectors creados:
if vector_positiu:
    desti = "fitxers/operadors/out/entrada_positiva/" + nom_graf + '.txt'
    for i in range(n):
        v = []
    ...
    desti = "fitxers/operadors/out/entrada_qualsevol/" + nom_graf + '.txt'
    for i in range(n):
        v = []
        for j in range(len(graf)):
...
for v in vectors:
    i_v = funcions.laplace_operator_out(graf, v)
    linia_e = 'e: (' + str(v[0])
    linia_s = 's: (' + str(i_v[0])
